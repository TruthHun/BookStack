...
## v2.3 升级日志
- [ ] 增加APP版本查询接口，以便提示安装了APP的用户升级到新版本（针对Android）
- [ ] 增加APP单页
- [ ] 用户签到功能
- [ ] APP 阅读时长统计
- [ ] 阅读活动以及阅读活动接口
...
- [ ] 管理后台删除用户，需要输入密码确认删除操作
- [ ] 用户注册接口优化，出现已存在用户名，直接返回字段错误了
- [ ] BookChatApp 用户退出登录再登录其他账号，书架收藏的书籍依然为上一个用户收藏的书籍（用户退出登录或者重新登录，需要重新变更书架状态）
- [ ] 支持电子书检测是否存在以及电子书重新生成

- [ ] 管理后台仪表盘增加程序环境监测，用以监控程序服务的相关文件夹权限和环境依赖是否部署正确
- [ ] 支持自动创建数据库，默认创建uft8mb4的数据库，避免用户创建了非utf8的数据库导致程序部署失败
- [ ] 管理后台支持标签管理（搜索、删除、新增）
- [x] 增加显示`我的收藏`入口
- [ ] 尽可能减少`app.conf`中的配置项，统一采用默认值
- [x] 增加广告投放功能
- [x] 移除`我的二维码`(这个功能意义不大)
- [x] 支持移除生成电子书时候的`statement.html`模板
