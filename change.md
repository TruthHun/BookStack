...
- [x] 文档阅读体验优化
    ...
    - [x] 章节目录跟随阅读进度定位位置
    - [x] 点击前进与后退的时候，没有重新加载文档（链接已变，但是内容没变）
- [x] 版本控制
    - [x] 文档编辑历史，使用文档的形式进行记录，不存入数据库.(版本控制，使用`<bookstack-git></bookstack-git>`标签对，提交版本说明)。
    - [x] 删除版本记录的时候要同时删除文档
    - [x] 删除文档项目时同时删除项目所有的相关文档章节记录
    - [x] 删除单个文档章节以及删除上一级文档章节，则被删除的文档章节的历史记录同样要删除
- [x] 文档编辑时各种模板可用模板
- [x] 书籍推荐，在封面加上"推荐"标志。
- [x] 分类在筛选文档项目的时候，SEO优化，以文档分类作为`title`等，不再以首页的文案作为`title`
- [x] 修复文档项目标识正则匹配规则不正确的问题
- [ ] 后端代码优化 —— 变量和函数，一会儿下划线一会儿驼峰交叉钩织的密集恐惧，看了让人有暴走的冲动，再加上比城市套路还深的`if else`嵌套...
...
- [ ] 文档中，图片不再使用带URL的绝对路径，以便以后更换域名，写死的URL找不到图片
    - [x] 编辑文档图片需要处理
    - [x] 阅读页面图片需要处理
    - [x] 导出文档图片需要处理
    - [x] 项目导入的图片需要处理
