...
- [x] `Git Clone` 的方式导入项目
- [x] 文档中，图片不再使用带URL的绝对路径，以便以后更换域名，写死的URL找不到图片
    - [x] 编辑文档图片需要处理
    - [x] 阅读页面图片需要处理
    - [x] 导出文档图片需要处理
    - [x] 项目导入的图片需要处理
- [x] 文档阅读体验优化
    ...
    - [x] 章节目录跟随阅读进度定位位置
- [x] 版本控制
    - [x] 文档编辑历史，使用文档的形式进行记录，不存入数据库.版本控制，在`<bookstack-git></bookstack-git>`标签内添加文案用以提交版本说明)。
    - [x] 删除版本记录的时候要同时删除文档
    - [x] 删除文档项目时同时删除项目所有的相关文档章节记录
    - [x] 删除单个文档章节以及删除上一级文档章节，则被删除的文档章节的历史记录同样要删除
- [x] `SUMMARY.md`功能扩展，输入`<bookstack-auto></bookstack-auto>`，获取所有文档标识
