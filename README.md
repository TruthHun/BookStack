...
踏入IT行业三年多时间以来，自身也受益于各种开源项目和开源组件，所以最终还是决定将BookStack开源出来。
...
大家在使用的时候，遇到问题，请随时到论坛反馈。我会接受大家的各种意见和建议，并把这个功能做完善和完美！
![一键导入项目](static/openstatic/import.png)


### 一键拉取GitHub上的markdown项目
看到GitHub项目上很多开源文档项目，但是一个一个去拷贝粘贴里面的markdown内容太麻烦了。于是，我做了这个一键拉取的功能。目前只有管理员才有权限拉取，并没有对普通用户开发。要体验这个功能，请用管理员账号登录演示站点体验。
用法很简单，比如我们拉取beego项目，在创建项目后，直接点击"拉取项目"，粘贴如"https://github.com/beego/beedoc/archive/master.zip"，然后就会自动帮你拉取上面的所有markdown文档并录入数据库，同时图片也会自动帮你更新到OSS。
![拉取项目](static/openstatic/pull.png)
Tips:
- 导入项目的时候，有时候markdown里面的图片路径替换不正确，需要手动更正过来。在oss上的存储规则，为`projects/你的项目的identify/你导入的项目的图片路径`
...
如果你知道站点的路由规则，倒是可以轻松链过去，但是，每次都要这样写，真的麻烦死了。我自己也经常写文档，简直受够了，然后想到了一个办法。如下：
我从`readme.md`跳转到`quickstart.md`，在`readme.md`中的内容这样写:
``` 
[快速开始]($quickstart.md)
```
好了，在发布文档的时候，文档就会根据路由规则以及你的文档标识去生成链接了。
